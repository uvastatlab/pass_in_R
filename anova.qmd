...
We will calculate power and sample size for a one-way ANOVA, a two-way ANOVA, and an ANCOVA. We will use base R, {pwr}, and {powertools}. 
...
We use `anova1way.F.bal` to calculate the overall sample size for balanced groups (each group has 20 people) 
...
Here, we will need to provide the effect size. We will first calculate it by hand: 
...
btwn.var.pop <- sum((book.means - mu)^2) / k 
...
Now, we will calculate sample size for a one-way ANOVA when we know the power we want to achieve, but and we need to determine the sample size
...
We now leave n (sample size) empty and input the power we would like to achieve. We will include a power of 0.8, indicating that if a difference actually exists between the mean and target, we have a 80% chance of detecting it.
...
Our estimated sample size is 20 per group.  
...
Our estimated sample size is ~20. 
...
Using {pwr2} for a balaned two-way ANOVA 
